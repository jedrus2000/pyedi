...
class Node(object):
    ...
    def _updatequerie(self, updatequeries):
        ...
        if updatequeries:
            if self._queries is None:
                ...
                updateunlessset(self._queries, updatequeries)
    ...
    def enhancedget(self, mpaths):
        ...
        if isinstance(mpaths, dict):
            ...
            raise MappingFormatError(
                _(
                    "Errors in mpath for QUERIES, SUBTRANSLATION or nextmessageblock; you used %(mpath)s."
                ),
                {"mpath": mpaths},
            )
    ...
    def _getrecordcore(self, mpaths):
        for key, value in mpaths[
            0
        ].items():  # check all key, value for first part of where;
            if key not in self.record or value != self.record[key]:
                return None  # no match:
        else:  # all key,value are matched.
            if (
                len(mpaths) == 1
            ):  # mpath is exhausted; so we are there!!! #replace values with values in 'change'; delete if None
                return self.record
            else:  # go recursive
                for childnode in self.children:
    ...
    def change(self, where, change):
        ...
        if not isinstance(change, dict):
            raise MappingFormatError(
                _(
                    'Parameter "change" must be dict: change(where=%(where)s,change=%(change)s)'
                ),
        ...
        for key, value in change.items():
            if not isinstance(key, str):
                raise MappingFormatError(
                    _(
                        'Keys in "change" must be strings: change(where=%(where)s,change=%(change)s)'
                    ),
                    {"where": where, "change": change},
                )
            if value is None:  # if None, item is deleted
                ...
                change[key] = str(
                    value
                ).strip()  # leading and trailing spaces are stripped from the values
    ...
    def delete(self, *mpaths):
        ...
        if len(mpaths) == 1:
            raise MappingFormatError(
                _(
                    "Only one dict: not allowed. Use different solution: delete(%(mpath)s)"
                ),
                {"mpath": mpaths},
            )
        for part in mpaths:
    ...
    def get(self, *mpaths):
        ...
        if Node.checklevel:
            ...
            if "BOTSID" not in mpaths[-1]:
                raise MappingFormatError(
                    _('Last section without "BOTSID": get(%(mpath)s)'),
                    {"mpath": mpaths},
                )
            count = 0
            for key, value in mpaths[-1].items():
                if not isinstance(key, str):
                    raise MappingFormatError(
                        _("Keys must be strings in last section: get(%(mpath)s)"),
                        {"mpath": mpaths},
                    )
                if value is None:
                    count += 1
                    ...
                    raise MappingFormatError(
                        _(
                            "Values must be strings (or none) in last section: get(%(mpath)s)"
                        ),
                        {"mpath": mpaths},
                    )
            if count > 1:
                raise MappingFormatError(
                    _('Max one "None" in last section: get(%(mpath)s)'),
                    {"mpath": mpaths},
                )
        for part in mpaths:
            if "BOTSIDnr" not in part:
    ...
    def _getloopcore(self, mpaths):
        ...
        for key, value in mpaths[0].items():
            ...
            if len(mpaths) == 1:
                ...
                for childnode in self.children:
                    for terug in childnode._getloopcore(
                        mpaths[1:]
                    ):  # search recursive for rest of mpaths
                        yield terug
    ...
    def getloop_including_mpath(self, *mpaths):
        ...
        for terug in self._getloopcore_including_mpath(mpaths):
            logmap.debug(
                'getloop %(mpaths)s returns "%(terug)s".',
                {"mpaths": mpaths, "terug": terug},
            )
            yield terug
    ...
    def put(self, *mpaths, **kwargs):
        ...
        for part in mpaths:
            ...
            for key, value in part.items():
                if value is None:
                    logmap.debug('"None" in put %(mpaths)s.', {"mpaths": str(mpaths)})
                    return False
                if not isinstance(key, str):
                    raise MappingFormatError(
    ...
    def _putcore(self, mpaths):
        ...
        for childnode in self.children:
            ...
            if childnode.record["BOTSID"] == mpaths[0][
                "BOTSID"
    ...
    def _mpath_sanity_check(mpaths):
        ...
        for part in mpaths:
            if not isinstance(part, dict):
                raise MappingFormatError(
                    _("Parameter mpath must be dicts in a tuple: %(mpaths)s"),
                    {"mpaths": mpaths},
                )
            if "BOTSID" not in part:
                raise MappingFormatError(
            ...
            for key, value in part.items():
                if not isinstance(key, str):
                    raise MappingFormatError(
                        _("Keys must be strings in mpath: %(mpaths)s"),
                        {"mpaths": mpaths},
                    )
                ...
                if (
                    ...
                    if mpaths[1:]:
                        if not LEVEL in record_definition:
                            return False
                        return _mpath_ok_with_grammar(
                            record_definition[LEVEL], mpaths[1:]
                        )
                    else:
