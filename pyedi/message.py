...
class Message(object):
    ...
    def __init__(self, ta_info):
        ...
        self.errorlist = (
            []
        )  # collect non-fatal errors in the edi file; used in reporting errors.
        self.errorfatal = (
            True
    ...
    def add2errorlist(self, errortxt):
        ...
        if len(self.errorlist) < config.get(["settings", "max_number_errors"]):
            self.errorlist.append(self.messagetypetxt + errortxt)
        elif len(self.errorlist) == config.get(["settings", "max_number_errors"]):
    ...
    def checkforerrorlist(self):
        ...
        if (
            self.errorfatal
        ):  # for fatal errors: (try to) get information like partners for edi file
    ...
    def messagegrammarread(self, typeofgrammarfile):
        ...
        updateunlessset(self.ta_info, self.defmessage.syntax)
    ...
    def _checkonemessage(self, node_instance, defmessage, subtranslation):
        ...
        self._checkifrecordsingrammar(
            node_instance, structure[0], defmessage.grammarname
        )
        self._canonicaltree(node_instance, structure[0])
        # should the content of the message (the records read) be logged.
        if not subtranslation and config.get(["settings", "readrecorddebug"]):
        ...
        for childnode in node_instance.children:  # for every record/childnode:
            for record_definition in structure[LEVEL]:  # search in grammar-records
                if childnode.record["BOTSID"] == record_definition[ID]:
                    ...
                    # check recursive:
                    self._checkifrecordsingrammar(
                        childnode, record_definition, grammarname
                    )
                ...
                if self.ta_info["checkunknownentities"]:
                    self.add2errorlist(
                        ...
                        % {
                            "linpos": node_instance.linpos(),
                            "record": childnode.record["BOTSID"],
                        }
                    )
        ...
        for field in list(
            ...
            for field_definition in record_definition[FIELDS]:
                ...
                if self.ta_info["checkunknownentities"]:
                    self.add2errorlist(
                        _(
                            '[F01]%(linpos)s: Record: "%(mpath)s" has unknown field "%(field)s".\n'
                        )
                        % {
                            "linpos": node_instance.linpos(),
                            "field": field,
                    ...
                    for comp in valuelist:
                        ...
                        for key, value in comp.items():
                            if not isinstance(key, str):
                                raise MappingFormatError(
                                    _(
                                        "Repeating composite: keys must be strings: put(%(mpath)s)"
                                    ),
                                    {"mpath": valuelist},
                                )
                            if value is None:
    ...
    def put(self, *mpaths, **kwargs):
        if self.root.record is None and self.root.children:
            raise MappingRootError(
                _(
                    'put(%(mpath)s): "root" of outgoing message is empty; use out.putloop'
                ),
                {"mpath": mpaths},
            )
